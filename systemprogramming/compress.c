...
int main(int argc, char *argv[])
{
	...
	switch(ftype[0])
	{
		case '1':
			...
			buffer=lz77compress(&buffer);
			buffer=lz77compress(&buffer);
			write_file(buffer, "1.cpd");
			break;
		case '2':
			buffer=get_buffer("2.html");
			result=lz77compress(&buffer);
			sz=file_sz;
			result=huffmancompress(&result, sz);
			write_file(result, "2.cpd");
			break;
		case '3':
			buffer=get_buffer("3.fna");
			buffer=changecompress(&buffer);
			write_file(buffer, "3.cpd");
			break;
		case '4':
			buffer=get_buffer("4.wav");
			sz=file_sz;
			result=huffmancompress(&buffer, sz);
			write_file(result, "4.cpd");
			break;
		case '5':
